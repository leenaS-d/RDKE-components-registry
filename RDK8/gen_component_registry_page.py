...
def build_components() -> None:
    ...
    body += f'''<style>.catalog-stats{{grid-template-columns:repeat(3,minmax(0,1fr))}}.catalog-toolbar{{display:grid;grid-template-columns:minmax(220px,2fr) repeat(2,minmax(160px,1fr));gap:12px;margin:20px 0}}.catalog-toolbar input,.catalog-toolbar select{{width:100%;min-width:0;margin:0}}@media(max-width:760px){{.catalog-stats,.catalog-toolbar{{grid-template-columns:1fr}}}}</style><section class="section"><div style="margin-bottom:24px">{release_panel("RDK8 catalog state", source)}</div><div class="stats catalog-stats"><div class="stat"><strong>{len(data)}</strong><span>Components</span></div><div class="stat"><strong>{len(categories)}</strong><span>Categories</span></div><div class="stat"><strong>{len(layers)}</strong><span>Layers</span></div></div><div class="toolbar catalog-toolbar"><input id="search" type="search" placeholder="Search components" aria-label="Search components"><select id="category"><option value="">All categories</option>{''.join(f'<option>{esc(item)}</option>' for item in categories)}</select><select id="layer"><option value="">All layers</option>{''.join(f'<option>{esc(item)}</option>' for item in layers)}</select></div><div class="table-wrap"><table><thead><tr><th>Component</th><th>Category</th><th>Layer</th><th>Type</th><th>Version</th><th>Source</th></tr></thead><tbody id="rows"></tbody></table></div></section>'''
